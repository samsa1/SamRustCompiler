...
fn rewrite_expr(top_expr : Expr, counter : &mut IdCounter) -> Expr {
    match *top_expr.content {
        ...
        ExprInner::Array(vec) => {
            Expr {
                content : Box::new(ExprInner::Array(vec.into_iter().map(|e| rewrite_expr(e, counter)).collect())),
                ..top_expr
            }
        },
        ...
        ExprInner::BinaryOp(op, expr1, expr2) => {
            Expr {
                content : Box::new(ExprInner::BinaryOp(op, rewrite_expr(expr1, counter), rewrite_expr(expr2, counter))),
                ..top_expr
            }
        },
        ...
        ExprInner::MacroCall(name, exprs) if name.get_content() == "vec" => {
            ...
            vec.push(Instr::Binding(true,
                ...
                Expr {
                    content : Box::new(ExprInner::FunCall(vec![], Ident::new("std::vec::Vec::new", top_expr.loc), Vec::new())),
                    loc : top_expr.loc,
                    typed : None,
                }
            ));
        ...
        ExprInner::BuildStruct(name, exprs) => {
            Expr {
                content : Box::new(ExprInner::BuildStruct(name,
                    exprs.into_iter().map(|(n, e)| (n, rewrite_expr(e, counter))).collect()
                    )),
                ..top_expr
            }
        },
        ...
        ExprInner::Tuple(exprs) => {
            Expr {
                content : Box::new(ExprInner::Tuple(
                    exprs.into_iter().map(|e| rewrite_expr(e, counter)).collect()
                    )),
                ..top_expr
            }
        },
