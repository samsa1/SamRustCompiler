...
fn eval(op : TBinop, v1 : &llr::Value, v2 : &llr::Value) -> Option<llr::Value> {
    ...
    match (op, v1, v2) {
        ...
        (TBinop::HArith(HArith {dm : HArithDesc::Mul, signed: false, size}), UInt(i1, _), UInt(i2, _)) =>
            Some(UInt((*i1 * *i2) as u64, size)),
        (TBinop::HArith(HArith {dm : HArithDesc::Mul, signed: true, size}), SInt(i1, _), SInt(i2, _)) =>
            Some(SInt(*i1 * *i2, size)),
        (TBinop::HArith(HArith {dm : HArithDesc::Div, signed: true, size}), SInt(i1, _), SInt(i2, _)) =>
            if v2.imm() == 0 { None } else {
                Some(SInt(*i1 / *i2, size))
            },
        (TBinop::HArith(HArith {dm : HArithDesc::Div, signed: false, size}), UInt(i1, _), UInt(i2, _)) =>
            if *i2 == 0 { panic!("ICE") } else {
                Some(UInt(*i1 / *i2, size))
            },
        (TBinop::HArith(HArith {dm : HArithDesc::Mod, signed: true, size}), SInt(i1, _), SInt(i2, _)) =>
            if v2.imm() == 0 { panic!("ICE") } else {
                Some(SInt(*i1 % *i2, size))
            },
        (TBinop::HArith(HArith {dm : HArithDesc::Mod, signed: false, size}), UInt(i1, _), UInt(i2, _)) =>
            if *i2 == 0 { panic!("ICE") } else {
                Some(UInt(*i1 % *i2, size))
            },
        ...
        (TBinop::Cmp(Cmp {cmp: CmpDesc::Eq, ..}), Bool(b1), Bool(b2)) => Some(Bool(*b1 == *b2)),
        (TBinop::Cmp(Cmp {cmp: CmpDesc::Eq, ..}), SInt(i1, _), SInt(i2, _)) => Some(Bool(*i1 == *i2)),
        (TBinop::Cmp(Cmp {cmp: CmpDesc::Eq, ..}), UInt(i1, _), UInt(i2, _)) => Some(Bool(*i1 == *i2)),
        (TBinop::Cmp(Cmp {cmp: CmpDesc::Neq, ..}), Bool(b1), Bool(b2)) => Some(Bool(*b1 != *b2)),
        (TBinop::Cmp(Cmp {cmp: CmpDesc::Neq, ..}), SInt(i1, _), SInt(i2, _)) => Some(Bool(*i1 != *i2)),
        (TBinop::Cmp(Cmp {cmp: CmpDesc::Neq, ..}), UInt(i1, _), UInt(i2, _)) => Some(Bool(*i1 != *i2)),
        (TBinop::Cmp(Cmp {cmp: CmpDesc::Lower, ..}), SInt(i1, _), SInt(i2, _)) => Some(Bool(*i1 < *i2)),
        (TBinop::Cmp(Cmp {cmp: CmpDesc::Lower, ..}), UInt(i1, _), UInt(i2, _)) => Some(Bool(*i1 < *i2)),
        (TBinop::Cmp(Cmp {cmp: CmpDesc::LowerEq, ..}), SInt(i1, _), SInt(i2, _)) => Some(Bool(*i1 <= *i2)),
        (TBinop::Cmp(Cmp {cmp: CmpDesc::LowerEq, ..}), UInt(i1, _), UInt(i2, _)) => Some(Bool(*i1 <= *i2)),
        (TBinop::Cmp(Cmp {cmp: CmpDesc::Greater, ..}), SInt(i1, _), SInt(i2, _)) => Some(Bool(*i1 > *i2)),
        (TBinop::Cmp(Cmp {cmp: CmpDesc::Greater, ..}), UInt(i1, _), UInt(i2, _)) => Some(Bool(*i1 > *i2)),
        (TBinop::Cmp(Cmp {cmp: CmpDesc::GreaterEq, ..}), SInt(i1, _), SInt(i2, _)) => Some(Bool(*i1 >= *i2)),
        (TBinop::Cmp(Cmp {cmp: CmpDesc::GreaterEq, ..}), UInt(i1, _), UInt(i2, _)) => Some(Bool(*i1 >= *i2)),
        (_, _, _) =>
