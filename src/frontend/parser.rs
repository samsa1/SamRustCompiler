...
peg::parser!{
    grammar list_parser() for str {
        ...
        rule reserved() =
            "true" / "false" / "if" / "else" / "while" / "fn" / "pub" / "struct" / "as" / "let"
        ...
        rule bloc() -> Bloc = precedence! {
            "{" space() "}" { Bloc { content : Vec::new() }}
            "{" space() b:bloc_inner() { Bloc {
                content : Vec::new()
            } }
        }
        ...
        rule instr() -> Instr = precedence! {
            e : expr() (quiet!{";"} / expected!("end of expr")) { println!("parsed Instr"); Instr::Expr(e) }
        ...
        rule expr() -> Expr = precedence! {
            "(" space() e:expr() space() ")" { println!("parsed 138 {:?}", e); e }
        ...
        rule expr_in() -> ExprInner = precedence! {
            ...
            e1:small_expr() space() lbracket() e2:expr_ws() "]" { println!("{:?}", e1); ExprInner::Method(e1, Ident::from_str("index"), vec![e2]) }
            n:name() space() "(" v:opt_expr_list() ")" { println!("call {:?}", n); ExprInner::FunCall(n, v)}
            "print" space() "!" space() "(" v:opt_expr_list() ")" { println!("print {:?}", v); ExprInner::MacroCall(Ident::from_str("print"), v) }
